...
@section('content')
    <div class="page-title-box">
        <div class="row align-items-center">
            <div class="col-sm-6">
                <h4 class="page-title">{{ __('custom.purchase_report') }}</h4>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="#" class="ic-javascriptVoid">{{ __('custom.purchase') }}</a></li>
                    <li class="breadcrumb-item active">{{ __('custom.purchase_report') }}</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="mt-0 header-title">{{ __('custom.purchase_report') }}</h4>
                    <div class="row">
                        <div class="col-sm-10">
                            <form action="{{ route('customer.reports.purchase') }}">
                    ...
                    @if ($data)
                        ...
                        __('custom.print') }}</button>
                            <a href="{{ route('customer.reports.export.purchase', ['type'=>'pdf', 'from_date'=>request()->from_date, 'to_date'=>request()->to_date]) }}"
                               class="btn btn-pdf btn-sm"> <i class="fa fa-file-pdf"></i> {{ __('custom.pdf') }}</a>
                            <a href="{{ route('customer.reports.export.purchase', ['type'=>'csv', 'from_date'=>request()->from_date, 'to_date'=>request()->to_date]) }}"
                               class="btn btn-success btn-sm"> <i class="fa fa-file-csv"></i> {{ __('custom.csv') }}</a>
                            <a href="{{ route('customer.reports.export.purchase', ['type'=>'excel', 'from_date'=>request()->from_date, 'to_date'=>request()->to_date]) }}"
                               class="btn btn-excel btn-sm"> <i class="fa fa-file-csv"></i> {{ __('custom.excel') }}</a>
                        </div>
                        <div id="section-to-print-sales">
                            <p class="mb-0"><b>{{ __('custom.purchase_report') }}:</b> {{ $report_range ?? '' }}</p>
                                ...
                                <table class="table table-sm table-bordered table-striped nowrap">
                                    ...
                                    @foreach ($data as $item)
                                        <tr>
                                            <td>{{ ++$loop->index }}</td>
                                            <td>
                                                <a href="{{ route('customer.invoices.show', $item->id) }}"
                                    ...
                                    <tfoot>
                                    <tr>
                                        <th colspan="6">{{ __('custom.total') }}</th>
